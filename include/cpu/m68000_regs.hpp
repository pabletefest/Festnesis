...
public:
  ...
  inline auto getDataReg(std::size_t index) -> std::uint32_t { return D_REGS[index]; }
  inline auto setDataReg(std::size_t index, std::uint32_t value) -> void { D_REGS[index] = value; }

  inline auto getAddressReg(std::size_t index) -> std::uint32_t { return A_REGS[index]; }
  inline auto setAddressReg(std::size_t index, std::uint32_t value) -> void { A_REGS[index] = value & 0x00FFFFFF; } // MSB is always 0 for m68k cpu
